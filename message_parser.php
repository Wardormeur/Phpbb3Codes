...
class bbcode_firstpass extends bbcode
{
	...
	function bbcode_init($allow_custom_bbcode = true)
	{
		...
		$this->bbcodes = array(
			...
			'flash'			=> array('bbcode_id' => 11,	'regexp' => array('#\[flash=([0-9]+),([0-9]+)\](.*?)\[/flash\]#uie' => "\$this->bbcode_flash('\$1', '\$2', '\$3')")),
                        'wiki'                  => array('bbcode_id' => 99,	'regexp' => array('#\[wiki](.+)\[/wiki\]#uiUe' => "\$this->bbcode_wiki('\$1')"))
		);
	...
	*/
	function bbcode_underline($in)
	{
        ...
        /**
	* Parse wiki tag -> im bored so that contact will come from here
	*/
	function bbcode_wiki($in)
	{
		if (!$this->check_bbcode('wiki', $in))
		{
			return $in;
		}
                // '
                //var_dump(file_get_contents( 'http://www.tellementnomade.org/wiki/doku.php?id='.$in.'&do=export_xhtmlcss') );
		return '[wiki='.$this->bbcode_specialchars($in).':'. $this->bbcode_uid . ']'. file_get_contents( 'http://www.tellementnomade.org/wiki/doku.php?id='.$in.'&do=export_xhtmlcss') .'[/wiki:' . $this->bbcode_uid . ']';
	}
